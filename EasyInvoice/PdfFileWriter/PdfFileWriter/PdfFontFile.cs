...
namespace PdfFileWriter
{
    internal class PdfFontFile : PdfObject
    {
        ...
        private readonly FontApi FontInfo;
        ...
        private readonly TableRecord[] TableRecordArray =
        {
        ...
        private void GetFontFileHeaderApi()
        {
            ...
            {
                FileVersion = ReadUInt32BigEndian(),
                NumTables = ReadUInt16BigEndian()
            };

            // number of bytes to retrieve
            var BufSize = 16 * FileHeader.NumTables;
            ...
            // load table records
            for (var Table = 0; Table < FileHeader.NumTables; Table++)
            {
                // get table tag (4 bytes)
                var TableTag = ReadUInt32BigEndian();
        ...
        private void GetheadTable()
        {
            ...
            {
                TableVersion = ReadUInt32BigEndian(),
                FontRevision = ReadUInt32BigEndian(),
                ChecksumAdjustment = ReadUInt32BigEndian(),
                MagicNumber = ReadUInt32BigEndian(),
                Flags = ReadUInt16BigEndian(),
                UnitsPerEm = ReadUInt16BigEndian(),
                TimeCreated = ReadInt64BigEndian(),
                TimeModified = ReadInt64BigEndian(),
                xMin = ReadInt16BigEndian(),
                yMin = ReadInt16BigEndian(),
                xMax = ReadInt16BigEndian(),
                yMax = ReadInt16BigEndian(),
                MacStyle = ReadUInt16BigEndian(),
                LowestRecPPEM = ReadUInt16BigEndian(),
                FontDirectionHint = ReadInt16BigEndian(),
                IndexToLocFormat = ReadInt16BigEndian(),
                glyphDataFormat = ReadInt16BigEndian()
            };
        ...
        private void GethheaTable()
        {
            ...
            {
                TableVersion = ReadUInt32BigEndian(),
                Ascender = ReadInt16BigEndian(),
                Descender = ReadInt16BigEndian(),
                LineGap = ReadInt16BigEndian(),
                advanceWidthMax = ReadUInt16BigEndian(),
                minLeftSideBearing = ReadInt16BigEndian(),
                minRightSideBearing = ReadInt16BigEndian(),
                xMaxExtent = ReadInt16BigEndian(),
                caretSlopeRise = ReadInt16BigEndian(),
                caretSlopeRun = ReadInt16BigEndian(),
                caretOffset = ReadInt16BigEndian(),
                Reserved1 = ReadInt16BigEndian(),
                Reserved2 = ReadInt16BigEndian(),
                Reserved3 = ReadInt16BigEndian(),
                Reserved4 = ReadInt16BigEndian(),
                metricDataFormat = ReadInt16BigEndian(),
                numberOfHMetrics = ReadUInt16BigEndian()
            };
        ...
        private void GetmaxpTable()
        {
            ...
            {
                TableVersion = ReadUInt32BigEndian(),
                numGlyphs = ReadUInt16BigEndian(),
                maxPoints = ReadUInt16BigEndian(),
                maxContours = ReadUInt16BigEndian(),
                maxCompositePoints = ReadUInt16BigEndian(),
                maxCompositeContours = ReadUInt16BigEndian(),
                maxZones = ReadUInt16BigEndian(),
                maxTwilightPoints = ReadUInt16BigEndian(),
                maxStorage = ReadUInt16BigEndian(),
                maxFunctionDefs = ReadUInt16BigEndian(),
                maxInstructionDefs = ReadUInt16BigEndian(),
                maxStackElements = ReadUInt16BigEndian(),
                maxSizeOfInstructions = ReadUInt16BigEndian(),
                maxComponentElements = ReadUInt16BigEndian(),
                maxComponentDepth = ReadUInt16BigEndian()
            };
        }
        ...
        private void GetlocaTable()
        {
            ...
            var TblSize = headTable.IndexToLocFormat == 0 ? Buffer.Length / 2 : Buffer.Length / 4;
        ...
        private void BuildGlyphArray()
        {
            ...
            var CompList = new List<int>();

            // create a glyph data list
            var GlyphList = CreateGlyphDataList(CompList);

            // loop for all possible characters of row zero
            var ZeroRow = CharInfoArray[0];
            for (var Col = FirstChar; Col <= LastChar; Col++)
            {
                // one char short cut
        ...
        {
            // create a glyph data list
            var GlyphList = new List<CharInfo>
            {
                // glyphs zero, one and two are reserved
                PdfFont.UndefinedCharInfo
            };
            AddGlyph(PdfFont.UndefinedCharInfo, GlyphList, CompList);

            var CharInfo = FontInfo.GetGlyphMetricsApiByGlyphIndex(1);
        ...
        )
        {
            ...
            // loop for all characters
            foreach (var GlyphIndex in ExtraList)
            {
        ...
        )
        {
            ...
            var GlyphLoc = locaTable[CharInfo.GlyphIndex];
            var GlyphLen = locaTable[CharInfo.GlyphIndex + 1] - GlyphLoc;
            ...
            // bounding box
            var xMin = (short) CharInfo.DesignBBoxLeft;
            var yMin = (short) CharInfo.DesignBBoxBottom;
            var xMax = (short) CharInfo.DesignBBoxRight;
        ...
        )
        {
            ...
            for (;;)
            {
                ...
                var Flags = (CompFlag) ReadUInt16BigEndian();
        ...
        private void BuildLocaTable()
        {
            ...
            var LocArray = new int[GlyphArray.Length + 1];

            // reset new glyph table length
            var GlyphTableLength = 0;
        ...
        private void BuildCharMapTable()
        {
            ...
            var NewSubTbl = new cmapSubTbl(cmapSubTbl.PlatformID, cmapSubTbl.EncodingID, 4)
            {
                Language = cmapSubTbl.Language,
                SegCount = 2,
                SegArray = new cmapSeg[2],
                GlyphArray = CharToGlyphArray
            };
            ...
            // table size
            var TblSize = 4 + 8 + 16 + 8 * NewSubTbl.SegCount + 2 * CharToGlyphArray.Length;
        ...
        private byte[] BuildEmbeddedFile()
        {
            ...
            foreach (var TR in TableRecordArray)
            {
                ...
                var Length = TR.Tag != glyfTag ? TR.Data.Length : TR.Length;
        ...
        {
            return ((uint) Buffer[BufPtr++] << 24) | ((uint) Buffer[BufPtr++] << 16) | ((uint) Buffer[BufPtr++] << 8) |
                   Buffer[BufPtr++];
        }
        ...
        {
            return ((uint) Buffer[BufPtr++] << 56) | ((uint) Buffer[BufPtr++] << 48) | ((uint) Buffer[BufPtr++] << 40) |
                   ((uint) Buffer[BufPtr++] << 32) |
                   ((uint) Buffer[BufPtr++] << 24) | ((uint) Buffer[BufPtr++] << 16) | ((uint) Buffer[BufPtr++] << 8) |
                   Buffer[BufPtr++];
        }
