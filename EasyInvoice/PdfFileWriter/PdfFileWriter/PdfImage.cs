...
namespace PdfFileWriter
{
    ...
    ///         <a href="http://www.codeproject.com/Articles/570682/PDF-File-Writer-Csharp-Class-Library-Version#ImageSupport">
    ///             2.4
    ///             Image Support
    ///         </a>
    ///     </para>
    ///     <para>
    ///         <a href="http://www.codeproject.com/Articles/570682/PDF-File-Writer-Csharp-Class-Library-Version#DrawImage">
    ///             For
    ///             example of drawing image see 3.9. Draw Image and Clip it
    ///         </a>
    ///     </para>
    /// </remarks>
    public class PdfImage : PdfObject, IDisposable
    {
        ...
        )
        {
            ...
            var DestRect = new Rectangle(0, 0, WidthPix, HeightPix);
            ...
            {
                // image resolution
                var ImageResolution = 0.5 * (HorizontalResolution + VerticalResolution);

                // requested resolution is less than image
                if (ImageControl.Resolution < ImageResolution)
                {
                    // change in resolution 
                    var Factor = ImageControl.Resolution / ImageResolution;

                    // convert to pixels based on requested resolution
                    var NewWidthPix = (int) (WidthPix * Factor + 0.5);
                    var NewHeightPix = (int) (HeightPix * Factor + 0.5);
            ...
            if (Image.GetType() == typeof(Bitmap))
            {
                ...
                else
                {
                    ...
                    var GR = Graphics.FromImage(Picture);
        ...
        internal void PictureToJpeg()
        {
            ...
            else
            {
                ...
                EncoderParameters.Param[0] = new EncoderParameter(Encoder.Quality, ImageControl.ImageQuality);
        ...
        internal bool PictureToIndexedImage()
        {
            ...
            try
            {
                ...
                PictureData = Picture.LockBits(new Rectangle(0, 0, WidthPix, HeightPix), ImageLockMode.ReadOnly,
            ...
            var FrameWidth = Math.Abs(PictureData.Stride);

            // number of unused bytes at the end of the frame
            var PicDelta = FrameWidth - 3 * WidthPix;

            // allocate byte array for picture bytes
            var PictureBytes = new byte[FrameWidth * HeightPix];

            // pointer to start of data in unmanaged memory
            var Scan0 = PictureData.Scan0;
            ...
            if (ColorArray.Count <= 2)
            {
                ...
                var WidthBytes = (WidthPix + 7) / 8;
                var ObjDelta = (WidthPix & 7) == 0 ? 0 : 1;
            ...
            else if (ColorArray.Count <= 4)
            {
                ...
                // each row must be multiple of bytes
                var WidthBytes = (WidthPix + 3) / 4;
                var ObjDelta = (WidthPix & 3) == 0 ? 0 : 1;
            ...
            else if (ColorArray.Count <= 16)
            {
                ...
                // each row must be multiple of bytes
                var WidthBytes = (WidthPix + 1) / 2;
                var ObjDelta = WidthPix & 1;
            ...
            var ColorByteArray = new byte[ColorArray.Count * 3];
            var ColorPtr = 0;
            ...
            // add items to dictionary
            Dictionary.AddFormat("/ColorSpace", "[/Indexed /DeviceRGB {0} {1}]", ColorArray.Count - 1,
                ColorStr); // R G B
        ...
        internal bool PictureToGrayImage()
        {
            ...
            for (var Y = 0; Y < HeightPix; Y++)
            {
                for (var X = 0; X < WidthPix; X++)
                {
                    ...
                    var Pixel = (11 * PictureBytes[PicPtr++] + 59 * PictureBytes[PicPtr++] +
                                 30 * PictureBytes[PicPtr++] + 50) / 100;
        ...
        internal bool PictureToBWImage()
        {
            ...
            var PicPtr = 0;
            var RowPtr = 0;
            var Cutoff = 255 * ImageControl._GrayToBWCutoff;
