    ...
    public class PdfRadialShading : PdfObject
    {
        ...
        ) : this(Document, 0.0, 0.0, 1.0, 1.0, new PdfShadingFunction(Document, MediaBrush))
        {
            SetGradientDirection(MediaBrush.Center.X, MediaBrush.Center.Y, 0.0,
                MediaBrush.GradientOrigin.X, MediaBrush.GradientOrigin.Y,
                0.5 * (MediaBrush.RadiusX + MediaBrush.RadiusY),
                MediaBrush.MappingMode == SysMedia.BrushMappingMode.RelativeToBoundingBox
        ...
        internal override void WriteObjectToPdfFile()
        {
            ...
            if (Mapping == MappingMode.Absolute)
            {
                Dictionary.AddFormat("/Coords", "[{0} {1} {2} {3} {4} {5}]",
                    ToPt(StartCenterX), ToPt(StartCenterY), ToPt(StartRadius), ToPt(EndCenterX), ToPt(EndCenterY),
                    ToPt(EndRadius));
            }
            // relative mapping mode
            else
            {
                var RelStartCenterX = BBoxLeft * (1.0 - StartCenterX) + BBoxRight * StartCenterX;
                var RelStartCenterY = BBoxBottom * (1.0 - StartCenterY) + BBoxTop * StartCenterY;
                var BBoxSide = Math.Min(Math.Abs(BBoxRight - BBoxLeft), Math.Abs(BBoxTop - BBoxBottom));
                var RelStartRadius = BBoxSide * StartRadius;
                var RelEndCenterX = BBoxLeft * (1.0 - EndCenterX) + BBoxRight * EndCenterX;
                var RelEndCenterY = BBoxBottom * (1.0 - EndCenterY) + BBoxTop * EndCenterY;
                var RelEndRadius = BBoxSide * EndRadius;
                Dictionary.AddFormat("/Coords", "[{0} {1} {2} {3} {4} {5}]",
                    ToPt(RelStartCenterX), ToPt(RelStartCenterY), ToPt(RelStartRadius), ToPt(RelEndCenterX),
                    ToPt(RelEndCenterY), ToPt(RelEndRadius));
            }

            // extend shading
            Dictionary.AddFormat("/Extend", "[{0} {1}]", ExtendShadingBefore ? "true" : "false",
                ExtendShadingAfter ? "true" : "false");
