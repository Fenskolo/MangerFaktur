...
namespace PdfFileWriter
{
    ...
    public class DrawWPFPath
    {
        ...
        )
        {
            ...
            if (NonStroking != null && (NonStroking.GetType() == typeof(SysMedia.LinearGradientBrush) ||
                                        NonStroking.GetType() == typeof(SysMedia.RadialGradientBrush) ||
                                        NonStroking.GetType() == typeof(PdfAxialShading) ||
                                        NonStroking.GetType() == typeof(PdfRadialShading)))
            {
                // save graphics state
                ...
                {
                    var AxialShading =
                        new PdfAxialShading(Contents.Document, (SysMedia.LinearGradientBrush) NonStroking);
                ...
                {
                    var RadialShading =
                        new PdfRadialShading(Contents.Document, (SysMedia.RadialGradientBrush) NonStroking);
                ...
                {
                    ((PdfRadialShading) NonStroking).SetBoundingBox(DrawRectX, DrawRectY, DrawRectWidth,
                        DrawRectHeight);
            ...
            if (Stroking != null)
            {
                ...
                {
                    // media pen short cut
                    var Pen = (SysMedia.Pen) Stroking;

                    // media brush shortcut
                    var Brush = (SysMedia.SolidColorBrush) Pen.Brush;

                    // media pen color short cut
                    var PenColor = Brush.Color;
        ...
        )
        {
            ...
            foreach (var SubPath in MediaPath.Figures)
            {
                // get start of sub-path point
                var CurPoint = PathToDrawing(SubPath.StartPoint);
                    ...
                    // line segment
                {
                    ...
                    {
                        var LineSegArray = (SysMedia.PolyLineSegment) Seg;
                        foreach (var PolyPoint in LineSegArray.Points)
                        {
                    ...
                    {
                        var BezierSeg = (SysMedia.BezierSegment) Seg;
                    ...
                    {
                        var BezierSegArray = (SysMedia.PolyBezierSegment) Seg;
                        var Count = BezierSegArray.Points.Count;
                        for (var Index = 0; Index < Count; Index += 3)
                        {
                            CurPoint = PathToDrawing(BezierSegArray.Points[Index + 2]);
                            Contents.DrawBezier(PathToDrawing(BezierSegArray.Points[Index]),
                                PathToDrawing(BezierSegArray.Points[Index + 1]), CurPoint);
                        }
                    }
                    ...
                    {
                        var BezierSeg = (SysMedia.QuadraticBezierSegment) Seg;
                        var NextPoint = PathToDrawing(BezierSeg.Point2);
                        Contents.DrawBezier(new BezierD(CurPoint, PathToDrawing(BezierSeg.Point1), NextPoint),
                    ...
                    {
                        var BezierSegArray = (SysMedia.PolyQuadraticBezierSegment) Seg;
                        var Count = BezierSegArray.Points.Count;
                        for (var Index = 0; Index < Count; Index += 2)
                        {
                            var NextPoint = PathToDrawing(BezierSegArray.Points[Index + 1]);
                            Contents.DrawBezier(
                                new BezierD(CurPoint, PathToDrawing(BezierSegArray.Points[Index]), NextPoint),
                                BezierPointOne.Ignore);
                            CurPoint = NextPoint;
                        }
                    }
                    ...
                    {
                        var Arc = (SysMedia.ArcSegment) Seg;
                        var NextPoint = PathToDrawing(Arc.Point);
                        ArcType ArcType;
        ...
        )
        {
            ...
            {
                var RatioWidth = DrawRectHeight * PathBBoxWidth / PathBBoxHeight;
