...
namespace PdfFileWriter
{
    ...
    /// </summary>
    /// <remarks>
    ///     The first three must be the same as TextJustify
    /// </remarks>
    ...
    ///         <a
    ///             href="http://www.codeproject.com/Articles/570682/PDF-File-Writer-Csharp-Class-Library-Version#CoordinateSystem">
    ///             2.1
    ///             Coordinate System
    ///         </a>
    ///     </para>
    ...
    public class PdfContents : PdfObject
    {
        private static readonly string[] PaintStr =
            {"", "n", "S", "s", "f", "f*", "B", "B*", "b", "b*", "h W n", "h W* n", "h"};
        ...
        )
        {
            ...
            else
            {
                ...
                foreach (var Value in DashArray)
                {
        ...
        {
            ObjectValueFormat("{0} {1} {2} rg\n", Round(Color.R / 255.0), Round(Color.G / 255.0),
                Round(Color.B / 255.0));
        }
        ...
        )
        {
            ...
            {
                AlphaStr = Alpha.ToString("0.0##", NFI.PeriodDecSep);
            }

            var ExtGState = PdfExtGState.CreateExtGState(Document, "/CA", AlphaStr);
        ...
        {
            var ExtGState = PdfExtGState.CreateExtGState(Document, "/BM", "/" + Blend);
            AddToUsedResources(ExtGState);
            ObjectValueFormat("{0} gs\n", ExtGState.ResourceCode);
        }
        ...
        )
        {
            ObjectValueFormat("{2} {3} {4} {2} {0} {1} cm\n",
                ToPt(OriginX), ToPt(OriginY), Round(Scale * Math.Cos(Rotate)), Round(Scale * Math.Sin(Rotate)),
                Round(Scale * Math.Sin(-Rotate)));
        }
        ...
        )
        {
            ObjectValueFormat("{2} {3} {4} {5} {0} {1} cm\n",
                ToPt(OriginX), ToPt(OriginY), Round(ScaleX * Math.Cos(Rotate)), Round(ScaleY * Math.Sin(Rotate)),
        ...
        )
        {
            ...
            // create arc
            var SegArray = ArcToBezier.CreateArc(ArcStart, ArcEnd, Radius, Rotate, Type);

            // output
            for (var Index = 1; Index < SegArray.Length;)
            {
                DrawBezier(SegArray[Index].X, SegArray[Index++].Y, SegArray[Index].X, SegArray[Index++].Y,
                    SegArray[Index].X, SegArray[Index++].Y);
            }
        }
        ...
        )
        {
            if (BorderStyle.Display)
            {
                ObjectValueFormat("q {0} w {1} {2} {3} RG 0 J {4} {5} m {6} {7} l S Q\n",
                    ToPt(BorderStyle.Width), Round(BorderStyle.Color.R / 255.0), Round(BorderStyle.Color.G / 255.0),
                    Round(BorderStyle.Color.B / 255.0), ToPt(X1), ToPt(Y1), ToPt(X2), ToPt(Y2));
            }
        }
        ...
        {
            // draw rectangle
            ObjectValueFormat("{0} {1} {2} {3} re {4}\n", ToPt(OriginX), ToPt(OriginY), ToPt(Width), ToPt(Height),
                PaintOpStr(PP));
        }
        ...
        ///             For
        ///             example of drawing heart see 3.10. Draw Heart
        ///         </a>
        ///     </para>
        /// </remarks>
        ////////////////////////////////////////////////////////////////////
        ...
        )
        {
            ...
            DrawBezier(new BezierD(CenterLine.P1, Factor1, -0.5 * Alpha1, Factor2, -0.5 * Alpha2, CenterLine.P2),
                BezierPointOne.MoveTo);
            DrawBezier(
                new BezierD(CenterLine.P2, Factor2, Math.PI + 0.5 * Alpha2, Factor1, Math.PI + 0.5 * Alpha1,
                    CenterLine.P1), BezierPointOne.Ignore);
        ...
        )
        {
            ...
            DrawBezier(BezierD.CircleFourthQuarter(OriginX + Width - Radius, OriginY + Radius, Radius),
                BezierPointOne.LineTo);
            DrawBezier(BezierD.CircleFirstQuarter(OriginX + Width - Radius, OriginY + Height - Radius, Radius),
                BezierPointOne.LineTo);
            DrawBezier(BezierD.CircleSecondQuarter(OriginX + Radius, OriginY + Height - Radius, Radius),
                BezierPointOne.LineTo);
        ...
        )
        {
            ...
            {
                throw new ApplicationException("Draw polygon error: path array must have at least two points");
            }
        ...
        )
        {
            ...
            {
                throw new ApplicationException("Draw polygon error: path array must have at least 4 items");
            }

            // program error
            if ((PathArray.Length & 1) != 0)
            {
                throw new ApplicationException("Draw polygon error: path array must have even number of items");
            }
            ...
            // draw lines		
            for (var Index = 2; Index < PathArray.Length; Index += 2)
            {
        ...
        )
        {
            ...
            // polygon angle
            var DeltaAlpha = 2.0 * Math.PI / Sides;
        ...
        )
        {
            ...
            else
            {
                ...
                // first line
                var L1 = new LineD(new PointD(Center, Radius, Alpha),
                    new PointD(Center, Radius, Alpha + 2.0 * DeltaAlpha));

                // second line
                var L2 = new LineD(new PointD(Center, Radius, Alpha - DeltaAlpha),
        ...
        )
        {
            ...
            if (Sides < 3)
            {
                throw new ApplicationException("Draw star. Number of sides must be 3 or more");
            }
        ...
        )
        {
            ...
            var FontSizeStr = string.Format(NFI.PeriodDecSep, "{0}", Round(FontSize));

            // set last use glyph index
            var GlyphIndexFlag = false;

            // text width
            var Width = 0;

            // loop for all text characters
            for (var Ptr = 0; Ptr < Text.Length; Ptr++)
            {
                // get character information
                var CharInfo = Font.GetCharInfo(Text[Ptr]);
        ...
        {
            var FontRes = new byte[ResCode.Length + SizeStr.Length + 4];
            var Index = 0;
            foreach (var TextChar in ResCode)
            {
                FontRes[Index++] = (byte) TextChar;
            }

            FontRes[Index++] = (byte) ' ';
            foreach (var TextChar in SizeStr)
            {
                FontRes[Index++] = (byte) TextChar;
            }
        ...
        )
        {
            ...
            var Width = DrawTextInternal(Font, FontSize, Text);
            EndTextMode();
        ...
        )
        {
            ...
            if ((DrawStyle & (DrawStyle.Subscript | DrawStyle.Superscript)) == 0)
            {
                ...
                if (DrawStyle != DrawStyle.Normal)
                {
                    ...
                    {
                        var UnderlinePos = PosY + Font.UnderlinePosition(FontSize);
                    ...
                    {
                        var StrikeoutPos = PosY + Font.StrikeoutPosition(FontSize);
        ...
        )
        {
            ...
            var Width = DrawTextWithKerning(Font, FontSize, KerningArray);
        ...
        )
        {
            ...
            var KernArray = Font.TextKerning(Text);
        ...
        )
        {
            ...
            // text width
            var Width = KernArray == null ? Font.TextWidth(FontSize, Text) : Font.TextKerningWidth(FontSize, KernArray);
        ...
        )
        {
            return DrawTextWithAnnotation(Page, Font, FontSize, TextAbsPosX, TextAbsPosY, TextJustify.Left,
                DrawStyle.Underline, Color.DarkBlue, Text, AnnotAction);
        }
        ...
        {
            return DrawTextWithAnnotation(Page, Font, FontSize, TextAbsPosX, TextAbsPosY, Justify, DrawStyle, TextColor,
                Text, new AnnotWebLink(WebLinkStr));
        }
        ...
        {
            var Width = DrawText(Font, FontSize, TextAbsPosX, TextAbsPosY, Justify, DrawStyle, TextColor, Text);
            if (Width == 0.0)
            {
                return 0.0;
            }
            ...
            var AnnotRect = new PdfRectangle(TextAbsPosX, TextAbsPosY - Font.DescentPlusLeading(FontSize),
                TextAbsPosX + Width, TextAbsPosY + Font.AscentPlusLeading(FontSize));
        ...
        )
        {
            ...
            {
                var SegWidth = DrawText(Seg.Font, Seg.FontSize, SegPosX, PosY, TextJustify.Left, Seg.DrawStyle,
                    Seg.FontColor, Seg.Text);
                if (Seg.AnnotAction != null)
                {
                    ...
                    var AnnotRect = new PdfRectangle(SegPosX, PosY - Seg.Font.DescentPlusLeading(Seg.FontSize),
        ...
        {
            if (!TextFitToWidth(Width, out var WordSpacing, out var CharSpacing, Line))
            {
            ...
            {
                var SegWidth =
                    DrawText(Seg.Font, Seg.FontSize, SegPosX, PosY, TextJustify.Left, Seg.DrawStyle, Seg.FontColor,
        ...
        {
            return DrawBarcode(PosX, PosY, TextJustify.Left, BarWidth, BarHeight, Color.Black, Barcode, TextFont,
                FontSize);
        }
        ...
        )
        {
            ...
            // barcode width
            var TotalWidth = BarWidth * Barcode.TotalWidth;
            ...
            {
                // loop for all bars
                for (var Index = 0; Index < Barcode.BarCount; Index++)
                {
                    // bar width in user units
                    var Width = BarWidth * Barcode.BarWidth(Index);
                ...
                if (TextFont != null)
                {
                    DrawBarcodeText(TextFont, FontSize, PosX + 0.5 * TotalWidth, PosY, TextJustify.Center,
                        Barcode.Text);
                }
            }
            ...
            else
            {
                ...
                for (var Index = 0; Index < Barcode.BarCount; Index++)
                {
                    ...
                    var DeltaY = Index < 7 || Index >= 27 && Index < 32 || Index >= 52 ? 0.0 : 5 * BarWidth;
                ...
                if (TextFont != null)
                {
                    ...
                    var PosX1 = PosX - 2.0 * BarWidth;
                    var PosX2 = PosX + 27.5 * BarWidth;
                    var PosX3 = PosX + 67.5 * BarWidth;
                    var PosX4 = PosX + 97.0 * BarWidth;
                    var PosY1 = PosY + 5.0 * BarWidth;

                    // UPC-A
                    if (Barcode.Text.Length == 12)
                    {
                        DrawBarcodeText(TextFont, FontSize, PosX1, PosY1, TextJustify.Right,
                            Barcode.Text.Substring(0, 1));
                        DrawBarcodeText(TextFont, FontSize, PosX2, PosY1, TextJustify.Center,
                            Barcode.Text.Substring(1, 5));
                        DrawBarcodeText(TextFont, FontSize, PosX3, PosY1, TextJustify.Center,
                            Barcode.Text.Substring(6, 5));
                    ...
                    else
                    {
                        DrawBarcodeText(TextFont, FontSize, PosX1, PosY1, TextJustify.Right,
                            Barcode.Text.Substring(0, 1));
                        DrawBarcodeText(TextFont, FontSize, PosX2, PosY1, TextJustify.Center,
                            Barcode.Text.Substring(1, 6));
                        DrawBarcodeText(TextFont, FontSize, PosX3, PosY1, TextJustify.Center,
                            Barcode.Text.Substring(7));
                    }
                }
            }
        ...
        )
        {
            if (ResObjects == null)
            {
                ResObjects = new List<PdfObject>();
            }

            var Index = ResObjects.BinarySearch(ResObject);
            if (Index < 0)
            {
                ResObjects.Insert(~Index, ResObject);
            }
        }
